 ...
 *
 */
public class IANSCUImpl implements IANSCU, MessageListener {

    public static final Logger LOG = LoggerFactory.getLogger(IANSCUImpl.class);
    ...
    public void init() throws JMSException {
        ...
        ianSCUQueue = jmsSession.createQueue("ianSCUQueue");
        jmsSession.createConsumer(ianSCUQueue).setMessageListener(this);
    }
    ...
    @Override
    public void createIAN(String localAET, String remoteAET, Attributes ian) {
        schedule(localAET, remoteAET, ian, 0, 0);
    }

    private void schedule(final String localAET, final String remoteAET,
            final Attributes ian, final int retries, long delay) {
        try {
            jmsService.sendMessage(ianSCUQueue, new MessageCreator() {
    ...
    private void process(ObjectMessage msg) throws JMSException {
        ...
        try {
            ...
            if (aeExt != null && retries < aeExt.getIANMaxRetries()) {
                ...
                schedule(localAET, remoteAET, ian, retries + 1, delay * 1000L);
            } else {
