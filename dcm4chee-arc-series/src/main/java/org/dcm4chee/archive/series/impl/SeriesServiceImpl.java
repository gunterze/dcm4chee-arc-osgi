...
public class SeriesServiceImpl implements SeriesService{
    ...
    public int[] calculateNumberOfSeriesRelatedInstances(Long seriesPk) {
        ...
        int numA = getEntityManager().createNamedQuery(Instance.NUMBER_OF_SERIES_RELATED_INSTANCES, Long.class)
                .setParameter(1, seriesPk)
                .setParameter(2, Availability.REJECTED_FOR_QUALITY_REASONS)
                .getSingleResult().intValue();
        getEntityManager().createNamedQuery(Series.UPDATE_NUMBER_OF_INSTANCES)
            .setParameter(1, num)
            .setParameter(2, numA)
    ...
    public int[] calculateNumberOfStudyRelatedInstances(Long studyPk) {
        int numSeries = getEntityManager().createNamedQuery(Series.NUMBER_OF_SERIES, Long.class)
                ...
                .setParameter(2, Availability.OFFLINE)
                .getSingleResult().intValue();
        int numSeriesA = getEntityManager().createNamedQuery(Series.NUMBER_OF_SERIES, Long.class)
                .setParameter(1, studyPk)
                .setParameter(2, Availability.REJECTED_FOR_QUALITY_REASONS)
                .getSingleResult().intValue();
        int numInstances = getEntityManager().createNamedQuery(
                    Instance.NUMBER_OF_STUDY_RELATED_INSTANCES, Long.class)
                .setParameter(1, studyPk)
                .setParameter(2, Availability.OFFLINE)
                .getSingleResult().intValue();
        int numInstancesA = getEntityManager().createNamedQuery(
                    Instance.NUMBER_OF_STUDY_RELATED_INSTANCES, Long.class)
                .setParameter(1, studyPk)
                .setParameter(2, Availability.REJECTED_FOR_QUALITY_REASONS)
                .getSingleResult().intValue();
        getEntityManager().createNamedQuery(Study.UPDATE_NUMBER_OF_INSTANCES)
